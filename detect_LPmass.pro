...
read, nthre, prompt = 'LP threshold factor = '
...
if sm_fact eq 1. then begin
save,CP,LP,featuresLP,masksLP,$
	min_size,dx,sigmaCP,sigmaLP,nx,ny,nt,nthreLP,filename=outputdir+strtrim(long(6302),2)+'_YAFTA_output_LP__'+strtrim(long(nthre),2)+'sigma__size'+strtrim(long(min_size),2)+'.sav'
endif else begin
save,CP,LP,featuresLP,masksLP,$
	min_size,dx,sigmaCP,sigmaLP,nx,ny,nt,nthreLP,filename=outputdir+strtrim(long(6302),2)+'_YAFTA_output_LP__'+strtrim(long(nthre),2)+'sigma__size'+strtrim(long(min_size),2)+'__smoothed.sav'
endelse
stop
end
