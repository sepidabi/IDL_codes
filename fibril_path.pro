...
for p = 0L, n_elements(map(*,0))/nxx -1 do begin
  ...
  cgimage,map(p*nxx:(p+1)*nxx-1,*),/scale,/keep_,/axes,/addcmd,axkeywords = axis(p),xrange = [0,nxx*res],yrange = [0,nyy*res],position = pos1[*,p], color = 'red temperature'
...
;stop
  ...
  if p eq 3 then begin
     ;cgplot, F_ca_x*res,F_ca_y*res,/addcmd,/noerase,color = 'red',thick = 1.5,position = pos1[*,0],/overplot,aspect=2./3 ;, linestyle = 2
...
;  stop
  ...
  if p eq 2 then cgtext,pos1(0,p)+1, pos1(1,p)+1, 'H$\alpha$', /add, color = 'white'
  if p eq 1 then cgtext,pos1(0,p)+1, pos1(1,p)+1, 'Ca II K', /add, color = 'white'
  if p eq 0 then cgtext,pos1(0,p)+1, pos1(1,p)+1, 'Ca II 8542 $\Angstrom$', /add, color = 'white'
  if p eq 3 then cgtext,pos1(0,p)+1, pos1(1,p)+1, 'Ca II K', /add, color = 'white'
  ...
  ; save the path map
cgcontrol,output = savedir+'path_'+strtrim(fn,2)+'_un_updated.eps',/ps_encap, ps_char = 0.7,/adjustsize, resize = [2*nxx,2*nyy+110]
